...
namespace Tests.EditMode.Chemistry
{
    public class FlaskTest
    {
        ...
        public void TestTransfer()
        {
            ...
            Assert.IsTrue(Flask<TestSubstance>.TryTransfer(f1, m0.ToFlask(), m0));
            Assert.AreEqual(m1.ToFlask(), f1);

            Assert.IsTrue(Flask<TestSubstance>.TryTransfer(f1, f2, m0));
            Assert.AreEqual(m1.ToFlask(), f1);
            Assert.AreEqual(m2.ToFlask(), f2);

            Assert.IsTrue(Flask<TestSubstance>.TryTransfer(f1, f2, m2));
            Assert.AreEqual((m1 + m2).ToFlask(), f1);
            Assert.AreEqual(m0.ToFlask(), f2);

            Assert.IsTrue(Flask<TestSubstance>.TryTransfer(f2, f1, m2));
            Assert.AreEqual(m1.ToFlask(), f1);
            Assert.AreEqual(m2.ToFlask(), f2);

            Assert.IsTrue(Flask<TestSubstance>.TryTransfer(f3, m3.ToFlask(), m3));
            Assert.IsTrue(Flask<TestSubstance>.TryTransfer(f1, f2, m2));
            Assert.IsTrue(Flask<TestSubstance>.TryTransfer(f1, f3, m3));
            Assert.AreEqual((m1 + m2 + m3).ToFlask(), f1);
            Assert.AreEqual(m0.ToFlask(), f2);
            Assert.AreEqual(m3.ToFlask(), f3);

            Assert.IsTrue(Flask<TestSubstance>.TryTransfer(f1, f1, f1));
            Assert.AreEqual((m1 + m2 + m3).ToFlask(), f1);

            // Insufficiency

            Assert.IsFalse(Flask<TestSubstance>.TryTransfer(f3, m0.ToFlask(), m3));
            Assert.AreEqual(m3.ToFlask(), f3);

            Assert.IsFalse(Flask<TestSubstance>.TryTransfer(f2, f3, m3 * 2));
            Assert.AreEqual(m3.ToFlask(), f3);
            Assert.AreEqual(m0.ToFlask(), f2);
        }
