...
namespace Tests
{
    public class MutationTest
    {
        ...
        {
            Random.InitState(1);    // sample ~= -0.99
            Assert.AreEqual(new int[] { -1 }, mutator.Mutate(new int[] { 0 }));

            Random.InitState(0);    // sample ~= -0.16
            Assert.AreEqual(new int[] { -2 }, mutator.Mutate(new int[] { -2 }));

            Random.InitState(2);    // sample ~=  0.17
            Assert.AreEqual(new int[] { 0 }, mutator.Mutate(new int[] { 0 }));

            Random.InitState(18);   // sample ~=  0.903
            Assert.AreEqual(new int[] { 0 }, mutator.Mutate(new int[] { -1 }));
        }
