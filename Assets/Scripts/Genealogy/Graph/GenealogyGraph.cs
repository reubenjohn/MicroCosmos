...
namespace Genealogy.Graph
{
    ...
    public partial class GenealogyGraph
    {
        ...
        public Node GetNodeOrDefault(Guid guid)
        {
            return nodes.TryGetValue(guid, out var node) ? node : default;
        }

        public List<Relation> GetRelationsFrom(Guid guid)
        {
            return relationsByFrom.TryGetValue(guid, out var relation) ? relation : null;
        }

        public List<Relation> GetRelationsTo(Guid guid)
        {
            return relationsByTo.TryGetValue(guid, out var relation) ? relation : null;
        }

        public Relation GetRelation(Guid from, Guid to)
        {
            return GetRelation(Tuple.Create(from, to));
        }

        public Relation GetRelation(Tuple<Guid, Guid> fromTo)
        {
            return relations.TryGetValue(fromTo, out var relation) ? relation : null;
        }
        ...
        {
            return RegisterReproductionAndOffspring(parents, (Node) child);
        }
