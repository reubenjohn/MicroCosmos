...
namespace Chemistry
{
    public static class ChemicalEquation
    {
        public class Term<T> : ImmutableSubstanceMass<T> where T : Enum
        {
            ...
            public static Expression<T> operator +(Term<T> a, Term<T> b) => new Expression<T>()
                {[a.substance] = a.mass, [b.substance] = b.mass};

            public static Reaction<T> operator >(Term<T> a, Term<T> b) => new Expression<T>() {[a.substance] = a.mass} >
